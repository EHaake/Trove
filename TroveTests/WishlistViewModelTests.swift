...
struct WishlistMarketSortTests {
    ...
    /// 003 Decision 12, the wishlist's half (see `ItemListViewModelTests`): the figure is older than thirty days,
    ...
    @Test func aFigureOlderThanThirtyDaysDrawsNoTrend() throws {
        let context = try makeInMemoryContext()
        insertWanted("Stale", into: context)
    ...
    @Test func aWithheldFigureDrawsNoTrend() throws {
        let context = try makeInMemoryContext()
        insertWanted("Withheld", into: context)

...
/// The wishlist form view model's stock-photo state and intents (spec 005,
/// plan §6), mirroring `ItemFormPhotoTests`. The form persists at its own
/// `save()`, so `store(_:)` only mutates the in-memory `photos` array. The
/// photo service is a spy — nothing here opens a connection — and the notice
/// flag is an in-memory fake, never the device's.
@Suite("WishlistFormViewModel — stock photo")
struct WishlistFormPhotoTests {
    ...
    ) throws -> WishlistFormViewModel {
        WishlistFormViewModel(

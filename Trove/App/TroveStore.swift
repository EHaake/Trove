...
/// One-time backfill giving items that predate `Item.sortOrder` (added in
/// `010`) a real manual order, per plan.md's Backfill migration section: a
/// schema default alone leaves every pre-existing item tied at 0, which is not
/// a manual order.
///
/// The flag flips once, after the save lands, and is never consulted for its
/// correctness again: the routine never re-derives or re-checks whether the
/// stored values "look backfilled." A heuristic that guesses wrong there is
/// quiet destruction of an order the user arranged by hand — the same shape
/// of quiet data loss this project has been burned by before (plan.md names
/// the photo-orphan defect). Setting the flag only *after* a successful save
/// is the other half of that: a failed backfill stays unclaimed, so the next
/// launch simply tries again.
///
/// `createdAt` is the seed order because it's the one attribute that isn't a
/// sort *mode* the user chooses between — just the order things were added, a
/// neutral starting point to rearrange from (plan.md).
enum ItemSortOrderBackfill {
    static let flagKey = "hasBackfilledItemSortOrder"

    /// The ephemeral store is skipped entirely and its launch never sets the
    /// flag — plan.md didn't anticipate this, but a `-uiTesting` launch that
    /// burned the flag against its throwaway store would stop the device's
    /// *real* store from ever being backfilled. The ephemeral store starts
    /// empty, so there is also nothing to backfill.
    static func runIfNeeded(
        mode: StorageMode,
        context: ModelContext,
        defaults: UserDefaults = .standard
    ) throws {
        guard mode != .ephemeral else { return }
        guard !defaults.bool(forKey: flagKey) else { return }

        let items = try context.fetch(
            FetchDescriptor<Item>(sortBy: [SortDescriptor(\.createdAt, order: .forward)])
        )

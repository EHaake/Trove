...
final class WishlistDetailViewModel {
    ...
    /// 34–35): the amount the person chose on the slider, written to what
    /// this is expected to cost. No fetch, and no history point — the
    /// market's record of itself is the median whatever is adopted.
    ///
    ...
    func adopt(cents: Int) -> Bool {
        ...
        // over the cost left as it was — and a gate that refuses the write
    ...
    /// commit leaves "Use as estimated cost" opening a sheet that isn't
    /// built yet.
    ///

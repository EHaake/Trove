...
**Status**: **Complete** (2026-09-18; criterion 12 attested by the person 2026-09-19, closing the spec's one partial — all fourteen criteria verified) — every task implemented and verified: T001–T011 with eleven sub-lettered additions (T001 fix, T009a–T009i from Decision 7's export scope and the anchor defect it uncovered, T010a from the device pass's criterion-3 finding, T010b from the Phase 2b sweep). **1540 unit tests in 208 suites** and **23 UI tests** green, the UI suite twice back to back. Thirteen of the fourteen acceptance criteria are ticked with per-criterion citations in `spec.md`; **one honest partial is named and stays unticked** — criterion 12's VoiceOver sweep with Accessibility Inspector is the person's own step, with what an agent could read of it (the swipe action's spoken name, from XCUITest) recorded beside it. Drafted by the `sdd-planner` at the top tier and signed off by the `skeptical-reviewer` at the top tier on 2026-09-16: three blocking findings (a hidden-narrowing leak into the Owned empty state, by-eye simulator steps in three tasks, a tie-break guard that could not reliably go red) fixed and re-reviewed in one round. The pre-merge sweep and marking the PR ready for review are the orchestrator's remaining steps.
...
- [x] **T010 — Device pass. [general-purpose agent with simulator tools; person: VoiceOver]**
...
- [x] **T010a — The meta line takes the header's full width, and two guards that can see it. `review: per-task`.**
  ...
  **Mutations, each recorded:** run G38 and G39 against the *unfixed* header
  ...
  every summary case red **including the Owned ones** (the prediction here
  said Owned would stay green; the measurement agreed with Q18's "the same
  latent wrap existed on the Owned side" instead, corrected at T010a); (b) render the baseline case at
...
- [x] **T010b — The three Phase 2b sweep items (guards only).**
...
| T010a — `sdd-implementer` (incl. one fix round) | `opus` | ~92k + ~140k | Done; clean orchestrator re-run green: UI 23 tests 0 failures, unit 1540 in 208 suites. Pre-fix red captured first and it reproduced the device pass exactly (G39: 168.0 vs 154.333, 13.667 pt apart; G38: 53 baseline vs 67 wrapped). Post-fix every case 53. Mutations: meta back in the leading `VStack` → every summary case red (Owned included — the task line's prediction was wrong and was corrected); baseline at width 200 → all red; `OverflowBadge` padding → 30 → the empty-trailing case red ALONE; the call site re-nested → the composition scan red, heights green. `ItemListSidesWiringTests` and `ImportWiringTests` green with no edit. `scripts/verify.sh`'s one-test count fix committed separately (`28b9dfa`) |
| T010a — `skeptical-reviewer` per-task review + re-review | `opus` | ~52k + ~68k | Two blocking, both fixed: a case that compared a measurement with itself (deleted), and the badge-height proviso asserted as measured when a uniform shift was invisible to every case (made true with an empty-trailing case, verified red-alone). Re-review: sign off, conditional on the orchestrator's clean re-run, which is green. At its direction plan Q18 now names the guard case and §10's G38 row lists all three triggers, and the script change was committed on its own |
| T010 re-measure after T010a — `general-purpose` with simulator tools | `opus` | ~430k (cumulative for the seat) | **Criterion 3 now passes.** Side-switch top edge 154.333 pt on BOTH sides in all three states — zero sales, `-seedSold`, and the person's 13-row set (the state that read 168.00 pt before). The wrap is visibly gone: the Sold summary is one band running to x=764 where it used to break at x=584 and spill a second line. Nothing else moved: title/badge row, meta, switch, search field and chip row land on identical rows on both sides, and the Owned side under its widest label (`Market ↓`) is unchanged; the Sold side's default `Date sold` is already the widest of its eight, so the table is that side's worst case. Build installed over the old one, the person's 7 owned / 6 sold intact, simulator left on the Items tab. **T010 is complete but for the person's Accessibility Inspector step (criterion 12)** — the tool's `inspect` is unavailable here, so it cannot come from an agent |
| T010b — `sdd-implementer` | `opus` | ~86k | Done; 1540 unit tests green, test-only changes. S2: a half-record case (sold date, no price) → the `?? 0` rewrite red, 4 issues, while both pre-existing entry tests stayed green — the review's finding exactly. S3: pinned the labelled pair as one literal on both screens (option A; the call sites are one source line and `SourceScan.production` only strips comments) → swapping the two closures red, 4 issues. S4: `realised == proceeds - paid` added to G33; honestly recorded as forward-looking — no mutation can break it while leaving the concrete triple intact, so it guards a future fixture rewrite rather than adding discrimination today |
| T011 — `sdd-implementer` (docs) | `opus` | ~171k | Done; documentation only. Criteria ticked with per-criterion citations, criterion 12 left an honest partial; the Copy section's placeholders replaced by the shipped strings; `plan.md` gains "As built"; `tokens.md`, `README.md`, `docs/csv-reference.md`, `ROADMAP.md` and `DECISIONS.md` corrected where `014` made them untrue. Finding: four citations named suites that do not exist (the test file holds several suites per concern) — corrected against the source; a future close-out bundle should carry suite names, not just test names |
| Pre-merge sweep — `skeptical-reviewer` | `opus` | ~248k | Two blocking, both about a claim naming a guard that cannot catch it being false: criterion 2 cited a test as enumerating the sold branch (no test does), and the swipe's spoken name was asserted as settled in three documents while the UI test matched either name. Second-look S1, S4, S5, S6, S9 actioned; S7 (a file-wide scan the plan scoped to the header), S8 (a badge height stated in three places, none of them a test) and S10 (`ExportScope`'s unpinned raw values) stand as recorded notes. Confirmed void: T009g's descendant-anchor worry (all eight `.dropdownAnchor(` call sites are leaf badges) |
| Sweep fixes — `sdd-implementer` | `opus` | ~217k | Both blocking fixed. Criterion 2 now cites the diff and states the enumeration gap rather than claiming it away. The swipe test pins `"Mark as sold…"` alone; mutation: the `.accessibilityLabel` removed → red at `:1320` and `:1329`, production restored byte-for-byte. Five small corrections; a fulfilment pointer at `006` Decision 16 (deliberately without the "Superseded by" phrase — fulfilled, not reversed — with T001's Verify line explaining the counts) |
| Final verification at `b8cfc11` | — | — | `scripts/verify.sh all` twice back to back: run 1 UI 23 tests 0 failures, unit 1540 in 208 suites; run 2 identical. Both exit 0, no failing tests. Closes the sweep's S3 |
| Person's attestation, 2026-09-19 | — | — | Walked the app on the simulator with the 13-row dataset: the swipe's Sell, the Sold side's search, chips and eight sorts, per-side state across a switch, the sold page's mark, and the export scope chooser in both formats — all reported working. Accessibility Inspector over the Sold side's field, chips and sort badge, the chooser's focus and dismiss catcher, and the header's new reading order: **all items pass**, the reading order accepted as it stands. Criterion 12 ticked; the spec closes with no honest partials |

...
**Phase 2 review** — after T004: one `skeptical-reviewer` pass at its default tier over `git diff <T002's commit>..HEAD` restricted to T004 (T003 was reviewed on its own as `review: per-task`), the task lines, plan §§4–7 and Q5–Q8, criteria 2, 3, 4, 10. Then the **Phase 1/Phase 2 boundary run** and the **Phase 2 pause**: the orchestrator runs `scripts/verify.sh ui` once after T003 lands (the launch path gained a branch), and after T005 twice; the person's report; a fresh session resumes at T006.

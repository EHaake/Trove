...
| **◆ Experiment 1 — close-out reading, 2026-09-13 (merge)** | session `claude-fable-5-1` @ medium, unchanged throughout; fallback never triggered | Fable allowance **32 % used / 68 % available** (the 2026-09-11 header row recorded "92 %"; the person notes three projects were drawing on the same allowance during the stretch — recorded as read, for the experiment's coordinator to reconcile, not interpreted here) | `ccusage session --breakdown` for this session's transcript from the experiment row to the merge: the Fable seat **4.48 M tokens** (4.30 M cache read = 96 %, 145 k cache write, 41 k output) ≈ **$5.79**; the Opus subagent rows in the same transcript 79.5 M tokens ≈ $58.69 (this row also carries the session's own Opus turns from Phases 1–3b, which the transcript doesn't separate). The seat is ~5 % of the stretch's tokens and ~9 % of its dollars. Pause-report readability: no verdict given; no report was sent back for rewording. Procedural misses by the session: none of the four the policy names (no skipped review, no stale `tasks.md` edit, no task done by hand, no in-place diagnosis); three bundle-hygiene misses recorded — the Phase 4 review bundle's spec sections came out empty (awk anchors), the diagnosis bundle named `Photo.swift` for a type that lives in `StockPhotoService.swift`, and the T013 bundle named 4 of 20 record-construction sites. One policy gap: the `sdd-implementer` has no simulator tools, so the device pass and its two follow-up checks ran in a `general-purpose` agent at `opus` (three dispatches, ~580 k tokens, screenshot-heavy). |
